...
def printBarCode(zipCode):
    ...
    if not zipCode.isdigit():
        print("Input is invalid, must be a 5 digit number.")
        return        
        ...
        while total > mult:
            mult += 10
        check = mult - total
        print ("|" + printDigit(zipCode[0]) + printDigit(zipCode[1]) + printDigit(zipCode[2]) + printDigit(zipCode[3]) + printDigit(zipCode[4]) + printDigit(str(check)) + "|")
    return
